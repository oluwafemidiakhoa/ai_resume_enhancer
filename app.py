...
def generate_prompt(profile_text, section):
    template = f"""
    You are an AI designed to read, analyze, and provide optimization suggestions for resumes.
    Given a resume, generate a detailed and professional report tailored to the resume owner.
...
# Main function
def main():
    st.set_page_config(page_title="Resume Enhancer", page_icon="📄", layout="centered")
    st.title("📄 AI Resume Enhancer")
    st.markdown("""
    ### Welcome to the AI Resume Enhancer!
    Upload a PDF of your resume, and we’ll analyze it to provide you with a comprehensive summary and suggestions for improvement.
    - The report will include analysis and suggestions for better keyword usage, content formatting, and section improvements.
    - You’ll receive clear, actionable steps to enhance your resume.
    """)
    ...
    if pdf_file is not None:
        ...
        if st.button("Get Summary Enhancement Suggestions"):
            generate_suggestions(text, "Summary Enhancement")
        if st.button("Get Work Experience Suggestions"):
            generate_suggestions(text, "Work Experience")
        if st.button("Get Education Section Suggestions"):
            generate_suggestions(text, "Education Section")
